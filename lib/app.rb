...
# Sélectionne dans le tableau 5 journalistes aléatoire.
tweet_journalist = 5.times.map {
   journalist[rand(0...journalist.length)] 
  }.uniq

# Tweet les 5 journalistes sélectionnés.
tweet_journalist.each {
   |x| connect_N.update("Hey guy nice last article #{x} ! #Hello_word  @the_hacking_pro")
  }

# Like et Follow les tweets contenant #Hello Word
connect_N.search("#Hello_Word", result_type: "recent").take(20).collect do |tweet|
 connect_N.favorite tweet
 connect_N.follow tweet.user
end

#Like et Follow en temp réel (streaming).
connect_S.filtrer(track: "#Hello_Word") do |object|
 connect_S.favorite object
 connect_S.follow object.user
end
